	...
	*/
	...
	function darDemandaValorOptimo ($rondas, $ronda, $tmpArrayVendedor, $idCampo, $tipoProducto) {
		...
		if($tipoProducto == 'firme' || $tipoProducto == 'firmeC') {
			$tmp = $this->calculaValorSiguienteYAgregado($tmpArrayVendedor, 'firme', $valorSiguiente, $valorActual, $valorSiguienteAgregado, $idCampo);
			$valorSiguiente = $tmp[0];
			$valorSiguienteAgregado = $tmp[1];
			
			$tmp = $this->calculaValorSiguienteYAgregado($tmpArrayVendedor, 'firmeC', $valorSiguiente, $valorActual, $valorSiguienteAgregado, $idCampo);
			$valorSiguiente = $tmp[0];
			$valorSiguienteAgregado = $tmp[1];
		}
		elseif($tipoProducto == 'condicional' || $tipoProducto == 'condicional') {
			...
			$tmp = $this->calculaValorSiguienteYAgregado($tmpArrayVendedor, 'condicional', $valorSiguiente, $valorActual, $valorSiguienteAgregado, $idCampo);
			$valorSiguiente = $tmp[0];
			$valorSiguienteAgregado = $tmp[1];
			
			$tmp = $this->calculaValorSiguienteYAgregado($tmpArrayVendedor, 'condicionalC', $valorSiguiente, $valorActual, $valorSiguienteAgregado, $idCampo);
			$valorSiguiente = $tmp[0];
			$valorSiguienteAgregado = $tmp[1];
		}
		elseif($tipoProducto == 'opcional' || $tipoProducto == 'opcionalU') {
			...
			$tmp = $this->calculaValorSiguienteYAgregado($tmpArrayVendedor, 'opcional', $valorSiguiente, $valorActual, $valorSiguienteAgregado, $idCampo);
			$valorSiguiente = $tmp[0];
			$valorSiguienteAgregado = $tmp[1];
			
			$tmp = $this->calculaValorSiguienteYAgregado($tmpArrayVendedor, 'opcionalC', $valorSiguiente, $valorActual, $valorSiguienteAgregado, $idCampo);
			$valorSiguiente = $tmp[0];
			$valorSiguienteAgregado = $tmp[1];			
		}
		
		echo $idCampo . "</br>";
		echo $productoReal . "</br>";
		echo $valorActual . "</br>";
		echo $valorSiguiente . "</br>";
		echo $valorSiguienteAgregado . "</br> </br>";
	 ...
	 */
	function calculaValorSiguienteYAgregado($tmpArrayVendedor, $tipoProducto, $valorSiguiente, $valorActual, $valorSiguienteAgregado, $idCampo) {
		foreach ($tmpArrayVendedor as $vendedor) {
			//Se saca los valores de la empresa
			$idCampoEmpresa = $vendedor['id'];
			$tmp = explode("|", $idCampoEmpresa);
